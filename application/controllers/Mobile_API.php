    ...
    */
	...
	public function customers_add(){
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST')
        {
            ...
            {
                //if the insert has returned true then we show the flash message
                if($customer_id = $this->customers_model->store_customer()){
                    $data = array();
					$data['status'] = 1;
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST' && (int)$customer_id > 0)
        {
            ...
            if ($this->form_validation->run())
            {
				...
				$new_member_insert_data = array(
					...
					'cst_date' => date("Y-m-d",strtotime($this->input->post('cst_date'))),
					'gst_number' => $this->input->post('gst_number'),
					'gst_date' => date("Y-m-d",strtotime($this->input->post('gst_date'))),
					'created_at' => date("Y-m-d H:i:s")
				);
				
				if($file_data["full_path"] != "")
					$new_member_insert_data["photo"]= $file_data["full_path"];
