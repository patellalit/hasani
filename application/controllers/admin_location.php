...
class Admin_location extends CI_Controller {
    ...
    */
    public function index()
    {
        ...
        $gets = $_GET;
        unset($gets['per_page']);
        $config['base_url'] = base_url().'admin/location/page?'.http_build_query($gets);
    ...
    public function location_csv()
    {
        ...
        //echo "<pre>";print_r($data['dsr']);exit;
        $config['total_rows'] = $data['count_location'];
        $file = fopen("location.csv","w");
        fputcsv($file,array('Id','Customer name','Address','Date'));
        
        foreach ($data['location'] as $line)
        {
            $arr = array($line['id'],$line['from_user_first_name'].' '.$line['from_user_last_name'],$line['address'],date('d/m/Y h:i A',strtotime($line['location_datetime'])));
            fputcsv($file,$arr);
            
        }
        
        fclose($file);
        echo $filename = "location.csv";exit;
