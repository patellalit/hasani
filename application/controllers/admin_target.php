...
class Admin_target extends CI_Controller {
    ...
    */
    ...
    public function target_csv()
    {
        ...
        $config['total_rows'] = $data['count_target'];
        $file = fopen("target.csv","w");
        fputcsv($file,array('','User name','Customer name','Date'));
        
        foreach ($data['target'] as $line)
        {
            $arr = array($line['id'],$line['user_name'],$line['customer_name'],$line['customer_name'],date('d-m-Y',strtotime($line['modified_at'])));
            fputcsv($file,$arr);
            
        }
        
        fclose($file);
        echo $filename = "target.csv";exit;
