...
class Admin_state extends CI_Controller {
    ...
    */
    ...
    public function state_csv()
    {
        ...
        $file = fopen("states.csv","w");
        fputcsv($file,array('','State','Country'));
        
        foreach ($data['state'] as $line)
        {
            $arr = array($line['id'],$line['name'],$line['country_name']);
            fputcsv($file,$arr);
            
        }
        
        fclose($file);
        echo $filename = "states.csv";exit;
        //initializate the panination helper 
        //$this->pagination->initialize($config);
